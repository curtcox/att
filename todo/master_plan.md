...
- [x] Completed additional adapter-session listing assertion helper reuse in NAT filter/freshness coverage.
  Reused combined helper/vector assertions by migrating the single-list freshness path, the `limit=1` filter path, and freshness-filter stale/unknown assertions (`test_manager_adapter_session_freshness_semantics`, `test_manager_list_adapter_sessions_supports_filters_and_limit`, `test_manager_list_adapter_sessions_supports_freshness_filter`) away from inline shape checks toward strict server-order + keyed-state assertions while preserving focused timeout-category regression semantics, invocation-event/connection-event filters, and call-order/subsequence behavior unchanged.
