...
- [x] Completed handoff productivity tooling for faster NEXT_MACHINE_HANDOFF read/update loops.
  Added `scripts/handoff_helper.py` (`quickview`, `prepend-recent`), new unit coverage `tests/unit/test_handoff_helper_script.py`, Makefile shortcuts (`handoff-quickview`, `handoff-prepend`), and explicit AGENTS self-guidance to use these commands when helpful, while preserving existing snapshot workflow, planning-doc guardrails, and full validation green.
