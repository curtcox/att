...
- [x] Completed failure-script setup/ordering/snapshot helper vectorization plus residual NAT adapter-session diagnostics/listing state-vector helper reuse (including strict server-order + keyed-state active-list checks via combined helper assertions, including pre-invocation inactive A/B listing checks) in nearby cluster NAT helper coverage:
