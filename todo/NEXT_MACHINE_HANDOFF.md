...
- Date: `2026-02-14`
- Branch: `main`
- HEAD: `d14a7e38463f26b22c600c220c2a6658012f8f01`
- Last commit: `d14a7e3 2026-02-14 10:26:30 -0600 - Refactor timeout-toggle setup in NAT exhaustion tests`
...
- Completed per-method invoke branch helper reuse in failure-script exhaustion coverage:
  - added local async unit-test helper `_invoke_unit_test_method_with_preferred(...)` in `tests/unit/test_mcp_client.py`.
  - migrated repeated per-method invoke branch wrappers (`read_resource` vs `invoke_tool`) in `test_cluster_nat_failure_script_exhaustion_falls_back_to_set_toggles` to helper-driven form while keeping method-branch semantics explicit and unchanged.
...
1. Reuse shared expected vectors in `tests/unit/test_mcp_client.py` where this improves consistency in nearby NAT helper tests:
   - migrate any remaining nearby repeated per-method invoke branch wrappers (`read_resource` vs `invoke_tool`) in retry-window/unreachable tests into shared local helper form where this keeps branch intent explicit and behavior unchanged.
