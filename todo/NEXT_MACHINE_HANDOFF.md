...
- Date: `2026-02-14`
- Branch: `main`
- HEAD: `23a8611cc65672820bb7a86a6a3c5a986576d0b6`
- Last commit: `23a8611 2026-02-14 13:56:57 -0600 - Reuse adapter diagnostics state-vector assertion helper`
...
1. Reuse shared expected vectors in `tests/unit/test_mcp_client.py` where this improves consistency in nearby NAT helper tests:
   - migrate any remaining nearby repeated adapter-session diagnostics/listing state assertions (especially post-invalidation/failure active-state checks, pre-invocation inactive listings, adapter-level diagnostics retrieval/state scaffolding, absent-manager diagnostics checks, and remaining single-use wrappers) into shared helper/vector form, preferring combined helper checks where strict server-order + keyed-state assertions appear together.
